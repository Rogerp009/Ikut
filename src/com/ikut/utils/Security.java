...
public final class Security {
     ...
     * Seed to encryption.
     */
    private static final String SEED = "com.ikut";
    ...
    private static void appendHex(StringBuffer sb, byte b) {
        sb.append(Security.HEX.charAt((b >> 4) & 0x0f)).append(Security.HEX.charAt(b & 0x0f));
    }
    ...
    public static String encryptToHex(String cleartext) throws Exception {
        byte[] rawKey = Security.getRawKey(Security.SEED.getBytes());
        byte[] result = Security.encrypt(rawKey, cleartext.getBytes());
        return Security.toHex(result);
    }
    ...
    public static String decrypt(String encrypted) throws Exception {
        byte[] rawKey = Security.getRawKey(Security.SEED.getBytes());
        byte[] enc = Security.toByte(encrypted);
        byte[] result = Security.decrypt(rawKey, enc);
    ...
    public static String base64Decode(String base64) {
        return Security.toHexadecimal(Base64.decode(base64, Base64.NO_WRAP));
    }
    ...
    public static String toHex(String txt) {
        return Security.toHex(txt.getBytes());
    }
    ...
    public static String fromHex(String hex) {
        return new String(Security.toByte(hex));
    }
    ...
    public static String toHex(byte[] buf) {
        ...
        for (int i = 0; i < buf.length; i++) {
            Security.appendHex(result, buf[i]);
        }
